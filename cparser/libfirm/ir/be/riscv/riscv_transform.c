...
{
	return be_get_Start_proj(irg, &riscv_registers[REG_X2]);
}

ir_node *get_Start_zero(ir_graph *const irg)
{
	return be_get_Start_proj(irg, &riscv_registers[REG_X0]);
}
...
static ir_node *make_address(ir_node const *const node, ir_entity *const ent, int32_t const val)
{
	...
	return new_bd_riscv_la(dbgi, block, ent, val);
}
...
static ir_node *gen_Call(ir_node *const node)
{
	...
	reqs[n_riscv_jal_stack] = &riscv_single_reg_req_gp_x2;
...
static ir_node *gen_Return(ir_node *const node)
{
	...
	reqs[n_riscv_ret_stack] = &riscv_single_reg_req_gp_x2;

	in[n_riscv_ret_addr]    = be_get_Start_proj(irg, &riscv_registers[REG_X1]);
	reqs[n_riscv_ret_addr]  = &riscv_single_reg_req_gp_x1;
...
static ir_node *gen_Start(ir_node *const node)
{
	be_start_out outs[N_RISCV_REGISTERS] = {
		[REG_X0] = BE_START_IGNORE,
		[REG_X1]   = BE_START_REG,
		[REG_X2]   = BE_START_IGNORE,
	};
	for (size_t i = 0; i != ARRAY_SIZE(callee_saves); ++i) {
...
static ir_node *gen_Switch(ir_node *const node)
{
	...
	ir_node   *const la    = new_bd_riscv_la(dbgi, block,  entity, 0);
    ir_node   *const add    = new_bd_riscv_add(dbgi, block, sll, la);
    ir_node   *const load   = new_bd_riscv_lw(dbgi, block, nomem, add, NULL, 0);
